package com.asfusion.mate.utils.binding
{
     ...
     */
    public static function watch(host:Object, chain:Object,  handler:Function,  commitOnly:Boolean = false):SoftChangeWatcher
    {
        if (!(chain is Array))
            chain = [ chain ];
        ...
        if (chain.length > 0)
        {
            var w:SoftChangeWatcher = new SoftChangeWatcher(chain[0], handler, commitOnly, watch(null, chain.slice(1), handler, commitOnly));
            w.reset(host);
            return w;
        }
        else
     ...
     */
    public function SoftChangeWatcher(access:Object, handler:Function,
                                  commitOnly:Boolean = false,
                                  next:SoftChangeWatcher = null)
    {
        super();
     ...
     */
    private var next:SoftChangeWatcher;
    ...
    public function reset(newHost:Object):void
    {
        ...
        if (host != null)
        {
            ...
            for (p in events)
            {
                host.addEventListener(p, wrapHandler, false, EventPriority.BINDING, true);
            }
        }
